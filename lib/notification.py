...
class Live():
    def __init__(self, title: str, channel_title: str, url: str, start_time: int, thumbnail: str):
        self.title = title
    ...
    def create_embed(self):
        ...
        miko.add_field(name = "頻道:",value=self.channel_title)
...
class CheckView(View): 
    ...
    async def join(self):
        try:
            ...
            print("watch_list:",self.watch_list)
    ...
    async def add_in_query(self,interaction:Interaction):
        ...
        print("control_panel:",control_panels)
    ...
    async def remove_notification(self,interaction:Interaction):
        ...
        if not self.channels and not self.already_joined_channels: #當等待直播隊列與已加入頻道隊列為空時
            ...
            self.watch_list.pop(self.live.title)
        await interaction.response.send_message(f"已移除 {self.live.channel_title} 直播通知",ephemeral=True)

async def create_channel(platform: str, URL: str) -> Channel:
    if platform == "youtube":
        soup = BeautifulSoup(requests.get(URL).content, "html.parser")
        videos_html = requests.get(URL+"/videos").text
        ...
        try:
            latest_video_url = "https://www.youtube.com/watch?v=" + re.search('(?<="videoId":").*?(?=")', videos_html).group()
        except:
            pass
    else:
        pass
...
@tasks.loop(seconds = 900) #15分鐘檢測一次
async def checkforvideos(bot: commands.Bot, notification_channels: dict, players: dict, control_panels: dict,watch_list:dict, node: wavelink.Node):
    ...
    def create_new_video_embed(channel:Channel,title:str,thumbnail:str): 
        ...
        miko.add_field(name = "頻道:",value=channel.title)
    ...
    for key, value in notification_channels.items():
        ...
        videos_html = requests.get(channel_url+"/videos").text
        c:Channel = value['obj']
                                ...
                                if section_item['itemSectionRenderer']['contents'][0]['shelfRenderer']['content'].get('expandedShelfContentsRenderer') != None: #只有一個影片會出現
                                    ...
                                    for video in video_list:
                                        if video['gridVideoRenderer'].get('upcomingEventData') != None:
                                            ...
                                            videos.append(Live(title, c.title, url, start_time, thumbnail))
                                else: #一個以上影片會出現
                    ...
                    if watch_list.__contains__(video.title): #舊的直播
                        ...
                        for (id,guild) in value['channels'].items():
                            if not waiting_channels.__contains__(id): #新的頻道訂閱群
                                ...
                                watch_list[video.title].channels[id]['message'] = await channel.send(embed = video.create_embed(),view = watch_list[video.title])
                        print("waiting_channels_after:",waiting_channels)
                    else: #新的直播加入
                        watch_list[video.title] = CheckView(video,value['channels'],bot,node,watch_list,players,control_panels)
                        for (id,guild) in value['channels'].items():
                            channel = await bot.fetch_channel(guild['obj'].text_id)
                            watch_list[video.title].channels[id]['message'] = await channel.send(embed = video.create_embed(),view = watch_list[video.title])
        ...
        try:
            latest_video_url = "https://www.youtube.com/watch?v=" + re.search('(?<="videoId":").*?(?=")', videos_html).group()
            thumbnail = re.findall('(?<="url":").*?(?=")', videos_html)[2]
            title = re.findall('(?<="text":").*?(?=")', videos_html)[0]
            print(latest_video_url)
        except:
            continue
        if not c.latest_video == latest_video_url:
